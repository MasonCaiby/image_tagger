...
def add_to_pickles(proj_dir, pickle_path):
    try:
        with open(proj_dir+'image_dicts.txt', 'r+') as image_dirs:
            if pickle_path not in image_dirs.read():
                image_dirs.write(pickle_path)

    except OSError as e:
        if e.errno == 2:
            with open(proj_dir+'image_dicts.txt', 'w') as image_dirs:
                image_dirs.write(pickle_path)
        else:
            sys.exit("add_to_pickles got this error: {}".format(e))
...
def loop_images(proj_dir, directory, save_freq=100):
    ...
    add_to_pickles(proj_dir, pickle_path)
    print(image_labels)
...
#   python loop_images.py -proj_dir ./ -dir test_images
